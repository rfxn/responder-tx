...
if [ "$SUITE" = py ] || [ "$SUITE" = all ]; then
    for f in server-gate gen-notices gen-shelters gen-caltopo gen-feeds gen-history gen-crest-summary gen-roads-snapshot gen-cameras gen-crossings-status; do
