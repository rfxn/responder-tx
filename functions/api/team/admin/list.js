...
export async function onRequestGet(context) {
  ...
  if (!env || !env.TEAM) return json({ error: 'team relay not configured' }, 503, 'noindex, nofollow');
  if (!authorized(request, env)) return json({ error: 'forbidden' }, 403, 'noindex, nofollow');
